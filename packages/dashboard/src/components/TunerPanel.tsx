...
function writeModel(raw: any) {
  try {
    ...
    console.warn('[Guardian] Failed to save model thresholds', err);
  }
}
    ...
    <div style={{ border: '1px solid #eee', borderRadius: 8, padding: 12 }}>
      ...
      {confusion && (
        <div style={{ marginTop: 12, fontSize: 14 }}>
          ...
          <div>Total evals: {confusion.total}</div>
          <div>Classifier HIGH: {confusion.high} ({confusion.pctHighAgree} agree w/fast path)</div>
          <div>Classifier HIGH only: {confusion.highNoFast} ({confusion.pctHighDisagree} disagree)</div>
          <div>Fast path higher severity: {confusion.fastHigher} ({confusion.pctFastHigher} of evals)</div>
          <div>Overall agreement: {confusion.agree}</div>
        </div>
      )}
    </div>
  );
